...
type WorkerMessage = ProcessDataMessage | GetWindowMessage | ClearCacheMessage;
...
const processRawData = (rawData: any, selectedDate: string) => {
...
const getWindowedData = (
  ...
  const analogMetrics = cachedProcessedData.analogMetrics.map((metric) => {
    ...
    const threshold = isSecondView ? 1500 : 300;
  ...
  const digitalMetrics = cachedProcessedData.digitalMetrics.map((metric) => {
    ...
    const downsampled = downsampleData(
      windowedPoints.map((p) => ({ time: p.time, value: p.value })),
      threshold
    );
...
self.onmessage = (e: MessageEvent<WorkerMessage>) => {
  ...
  try {
    switch (message.type) {
      case 'PROCESS_DATA': {
        const { rawData, selectedDate, windowStart, windowEnd, isSecondView } = message.payload;

        // Check if we need to reprocess (new data or date change)
        ...
        if (needsReprocessing) {
          cachedProcessedData = processRawData(rawData, selectedDate);
          cachedRawData = rawData;
          cachedSelectedDate = selectedDate;
        }

        // Get windowed data if window specified, otherwise return downsampled full data
        let result;
        if (windowStart != null && windowEnd != null) {
          result = getWindowedData(windowStart, windowEnd, isSecondView);
        } else {
          // Return downsampled full dataset
          if (!cachedProcessedData) {
            self.postMessage({
              type: 'ERROR',
              payload: { message: 'No processed data available' },
            });
            return;
          }
          ...
          const threshold = isSecondView ? 1500 : 300;
          result = {
            analogMetrics: cachedProcessedData.analogMetrics.map((m) => ({
              ...m,
              data: downsampleAnalogData(m.allPoints, threshold).map((p) => ({
                time: new Date(p.time),
                avg: p.avg,
                min: p.min,
                max: p.max,
            ...
            digitalMetrics: cachedProcessedData.digitalMetrics.map((m) => ({
              ...m,
              data: downsampleData(
                m.allPoints.map((p: { time: number; value: number }) => ({ time: p.time, value: p.value })),
                threshold
              ).map((p) => ({
                time: new Date(p.time),
                value: p.value,
            ...
            gpsData: cachedProcessedData.gpsData.map((p) => ({ ...p, time: new Date(p.time) })),
            timestamps: cachedProcessedData.timestamps,
          };
        }
      ...
      case 'GET_WINDOW': {
        const { windowStart, windowEnd, isSecondView } = message.payload;
        const result = getWindowedData(windowStart, windowEnd, isSecondView);
        self.postMessage({
          type: 'WINDOW_DATA',
          payload: result,
        });
    ...
    self.postMessage({
      type: 'ERROR',
      payload: { message: error.message || 'Processing error', stack: error.stack },
    });
  }
};
